...
import { Loader } from './loader/loader2.js'
...
function Tileset (url, scene, camera, geometricErrorMultiplier, meshCallback) {
  ...
  this.camera = camera
  this.geometricErrorMultiplier = geometricErrorMultiplier || 1
  ...
  function update () {
    ...
    self.rootTile.getTilesInView(frustum, self.tilesetCamera.position, self.geometricErrorMultiplier, self.loadAroundView).then(tiles => {
      ...
      if (tiles.length > 0 && !!self.scene) {
        const newTilesContent = tiles.map(tile => tile.content)
        const toDelete = []
          ...
          if (!self.currentlyRenderedTiles[content]) {
            if (self.futureActionOnTiles[content] !== 'toUpdate') {
              ...
              contentRequests.push(
                self.cache.get(content, self.controller.signal).then(gltf => {
                  if (!!gltf && !!self.scene) {
                    if (self.futureActionOnTiles[content] === 'toUpdate') {
        ...
        if (contentRequests.length > 0) {
          ...
          Promise.all(contentRequests).catch(error => {
            ...
            if (!self.controller.signal.aborted && !!self.scene) {
  ...
  return {
    ...
    'apply': applyMovement,
    'matrix': self.matrix,
    'currentlyRenderedTiles': self.currentlyRenderedTiles,
    'rootJson': self.loader.rootJson,
