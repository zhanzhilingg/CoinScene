...
function OBB (values) {
  ...
  function distanceToPoint (point) {
    ...
    /// / point to bounds
    const dx = Math.max(0, Math.max(-self.halfWidth - transformedPoint.x, transformedPoint.x - self.halfWidth))
    const dy = Math.max(0, Math.max(-self.halfHeight - transformedPoint.y, transformedPoint.y - self.halfHeight))
    const dz = Math.max(0, Math.max(-self.halfDepth - transformedPoint.z, transformedPoint.z - self.halfDepth))
